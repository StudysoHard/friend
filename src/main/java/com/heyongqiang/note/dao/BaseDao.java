...
public class BaseDao {
    ...
    //查询一个字段  只会返回一条记录且只有一个字段 用于查询总数量
    public static Object findSingleVale(String sql, List<Object> params){
        Object object =null;
        Connection connection =null;
        PreparedStatement preparedStatement =null;
        ...
        try {
            ...
            while (resultSet.next()){
                ...
                for (int i =1;i<=fieldNum;i++){
                    //得到查询的每一个列名
                    //getColumnLabel():获取列名或者别名
                    //getColumnName():获取列名
                    String columnName =resultSetMetaData.getColumnLabel(i);
                    //通过反射，使用列名得到对应的fileid对象
                    Field field =cls.getDeclaredField(columnName);
                    //拼接set方法，得到字符串
                    String setMethod ="set"+columnName.substring(0,1).toUpperCase()+columnName.substring(1);
                    //通过反射，将set方法字符串放射成对应类中的set方法
    ...
    //进入自动查询所有数据
    public  static  List queryAll(String sql,List<Object> params,Class cls){
        List list =new ArrayList();
        Connection connection = null;
        PreparedStatement preparedStatement =null;
        ResultSet resultSet = null;
